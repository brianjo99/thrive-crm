...
import { Scissors, FolderOpen, CheckCircle } from "lucide-react";
import { motion } from "framer-motion";

export default function EditorDashboard() {
  const { getTasksForRole, campaigns } = useThriveStore();

  const editorTasks = getTasksForRole('editor');
  const pendingTasks = editorTasks.filter(t => t.status === 'pending' || t.status === 'in-progress');
  const completedTasks = editorTasks.filter(t => t.status === 'complete');
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <Card className="luxury-card p-5">
                ...
                <TaskList
                  tasks={pendingTasks}
                  emptyMessage="No editing tasks assigned to you"
                />
              </Card>
            </motion.div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {editorTasks.filter(t => t.status === 'in-progress').length}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Pending Review</span>
                    <span className="font-medium">
                      {editorTasks.filter(t => t.status === 'review').length}
                    </span>
                  </div>
                </div>
              </Card>
